...
function scrapeWebpages() {
	...
	catNames.forEach(async e => {
		const url = `https://allaboutcats.com/cat-breeds/${e.toLowerCase().replace(" ", "-")}`;
		...
		try {
			...
			datas.splice(9).forEach(e => characteristics.push(e.trim().split(" ")[1].split("%")[1] + "%"));
			...
			fs.writeFile("./src/database/json/cats.json", JSON.stringify(cats, null, 2), err => {
				if (err) {
					console.error(err);
					return;
				}
