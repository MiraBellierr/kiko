	...
	run: async (client, interaction) => {
		...
		if (search.length > 1) {
			const searchEmbed = new Discord.MessageEmbed()
				...
				.setDescription(`Found ${search.length} results\n\n- ${search.join("\n- ")}`)
				.setColor("#CD1C6C")
			...
			const page1 = new Discord.MessageEmbed()
				...
				.setFooter({ text: "Page 1/7 | allaboutcats.com", iconURL: client.user.displayAvatarURL({ dynamic: true }) });

			const page2 = new Discord.MessageEmbed()
				.setAuthor({ name: "Cat Breed Info", iconURL: interaction.user.displayAvatarURL({ dynamic: true })})
				.setTitle(`${cat.name} | Characteristics`)
				.addFields([
					{ name: "Affection Level", value: `${getProgbar(Number.parseInt(cat.characteristics.affectionLevel.split("%")[0]))}`},
					{ name: "Activity Level", value: `${getProgbar(Number.parseInt(cat.characteristics.activityLevel.split("%")[0]))}`},
					{ name: "Pet-Friendly", value: `${getProgbar(Number.parseInt(cat.characteristics.petFriendly.split("%")[0]))}`},
					{ name: "Kid-Friendly", value: `${getProgbar(Number.parseInt(cat.characteristics.affectionLevel.split("%")[0]))}`},
					{ name: "Sociability", value: `${getProgbar(Number.parseInt(cat.characteristics.sociability.split("%")[0]))}`},
					{ name: "Intelligence", value: `${getProgbar(Number.parseInt(cat.characteristics.intelligence.split("%")[0]))}`},
					{ name: "Playfulness", value: `${getProgbar(Number.parseInt(cat.characteristics.playfulness.split("%")[0]))}`},
					{ name: "Independence", value: `${getProgbar(Number.parseInt(cat.characteristics.independence.split("%")[0]))}`},
					{ name: "Vocality", value: `${getProgbar(Number.parseInt(cat.characteristics.vocality.split("%")[0]))}`},
					{ name: "Grooming", value: `${getProgbar(Number.parseInt(cat.characteristics.grooming.split("%")[0]))}`}
				])
				.setColor("#CD1C6C")
				.setTimestamp()
				.setImage(cat.images[0])
				.setFooter({ text: "Page 2/7 | allaboutcats.com", iconURL: client.user.displayAvatarURL({ dynamic: true }) });
			...
			const page3 = new Discord.MessageEmbed()
				...
				.setFooter({ text: "Page 3/7 | allaboutcats.com", iconURL: client.user.displayAvatarURL({ dynamic: true }) });

			const page4 = new Discord.MessageEmbed()
				.setAuthor({ name: "Cat Breed Info", iconURL: interaction.user.displayAvatarURL({ dynamic: true })})
				.setTitle(`${cat.name} | Care`)
				.setDescription(`**__Nutrition__**\n${cat.care.nutrition}\n\n**__Grooming__**\n${cat.care.grooming}\n\n**__Exercise__**\n${cat.care.exercise}\n\n**__Health__**\n${cat.care.health}`)
				.setColor("#CD1C6C")
				.setTimestamp()
				.setImage(cat.images[2])
				.setFooter({ text: "Page 4/7 | allaboutcats.com", iconURL: client.user.displayAvatarURL({ dynamic: true }) });
...
function getProgbar(current) {
	const curBer = Math.floor((current / 100) * 22);
	let str = "";
	for(let i = 0; i < 22; i++) {
		str += i < curBer ? "💙" : " ";
	}
