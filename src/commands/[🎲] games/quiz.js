...
module.exports = {
	...
	run: async (client, interaction) => {
		...
		const fetch = await axios.get("https://opentdb.com/api.php?amount=1&category=27");
		...
		if (i.customId === quiz.correct_answer) {
			Quiz().update({ point: point + 1 }, { where: { userid: interaction.user.id } });
            
			replyEmbed.setAuthor({ name: interaction.user.username, iconURL: interaction.user.displayAvatarURL({ dynamic: true }) });
			replyEmbed.setTitle(quiz.category);
			replyEmbed.setDescription(`${question}\n\nCorrect, the answer is ${decode(quiz.correct_answer)}`);
			replyEmbed.setColor("#CD1C6C");
			replyEmbed.setFooter({ text: `You get 1 point. Your current point is ${point + 1}` });
			...
			replyEmbed.setAuthor({ name: interaction.user.username, iconURL: interaction.user.displayAvatarURL({ dynamic: true }) });
			replyEmbed.setTitle(quiz.category);
			replyEmbed.setDescription(`${question}\n\nWrong, the correct answer is ${decode(quiz.correct_answer)}`);
			replyEmbed.setColor("#CD1C6C");
			replyEmbed.setFooter({ text: `-1 point penalty. Your current point is ${point - 1}` });
			replyEmbed.setTimestamp();
