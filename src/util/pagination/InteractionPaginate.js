...
function Paginate(client, interaction, pages, options = {
	...
	this.init = async function() {
		...
		const disabledForward = new MessageActionRow()
			.addComponents([
				new MessageButton()
					.setCustomId("backward")
					.setLabel("Back")
		...
		const disabledBackward = new MessageActionRow()
			.addComponents([
				new MessageButton()
					.setCustomId("backward")
					.setLabel("Back")
		...
		if (typeof this.pages[page - 1] == "object") {
			message = await this.interaction.reply({ embeds: [this.pages[page - 1]], components: [disabledBackward], fetchReply: true });
		}
		else {
			message = await this.interaction.reply({ content: `${[this.pages[page - 1]]}`, components: [disabledBackward], fetchReply: true });
		}
